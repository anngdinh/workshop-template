---
title: "Running Untrusted AI Code Safely"
subtitle: "Sandbox infrastructure — the landscape, secure container runtimes, OpenSandbox & Agent Sandbox"
# format:
#   revealjs:
#     theme: sky
...
1. **The problem & the landscape** — why sandboxes, and who's building them (E2B, Daytona, Vercel, Cloudflare, AWS…)
2. **Secure container runtimes** — Firecracker · gVisor · Kata, measured head-to-head
3. **OpenSandbox** — universal sandbox infrastructure for AI
4. **Agent Sandbox** — the Kubernetes-native `Sandbox` CRD (SIG Apps)
5. **Q & A**

# The Problem & The Landscape {background-color="#eef1f4"}

Why sandboxes — and who's building them

## AI writes code now — and runs it {background-color="#eef1f4" .smaller}

::: {.incremental}
- LLM agents **generate and execute** code, shell commands, and browser actions — on *your* infrastructure
- That code is **untrusted / adversarial by assumption**
- A plain container **shares the host kernel** — process isolation, not a security boundary
- One escape → other tenants, your secrets, the host
:::

::: {.fragment}
So you need a **sandbox**: isolate it, run it fast, tear it down.
:::

## What a sandbox must do {background-color="#eef1f4" .smaller}

::: {.incremental}
- 🔒 **Strong isolation** — contain hostile code
- ⚡ **Fast start** — agents spawn one per turn (ms, not seconds)
- ♻️ **Stateful lifecycle** — create / pause / resume / snapshot
- 🌐 **Controlled fs & network** — egress allowlists, no lateral movement
- 📈 **Scale & cost** — thousands of short-lived sandboxes, cheaply
:::

::: {.fragment}
These five tensions drive **every** design choice in this talk.
:::

## The landscape {background-color="#eef1f4" .smaller}

**Managed platforms (SaaS)**
E2B · Daytona · Modal · Vercel Sandbox · Cloudflare Sandbox SDK · AWS Bedrock AgentCore · CodeSandbox · Runloop

**Cloud primitives**
AWS Firecracker (Lambda / Fargate) · Fly Machines · GKE + gVisor · Cloud Run

**Isolation tech underneath**
Firecracker · gVisor · Kata · plain containers

::: {.fragment}
An explosion of options since 2024 — all solving the **same** problem.
:::

## Solutions at a glance {background-color="#eef1f4" .smaller .scrollable}

| Solution | What it is | Isolation | Model |
|---|---|---|---|
| **E2B** | OSS sandboxes for AI agents / code | Firecracker microVM (~150 ms) | OSS + cloud |
| **Daytona** | Fast AI code sandboxes / workspaces | Containers, shared kernel (~90 ms) | Managed |
| **Modal** | Serverless compute + Sandboxes API | gVisor | Managed |
| **Vercel Sandbox** | Ephemeral compute for untrusted code | Firecracker microVM | Managed (GA 2026) |
| **Cloudflare Sandbox SDK** | Code execution from Workers | Containers + V8 isolates, edge | Managed |
| **AWS Bedrock AgentCore** | Managed agent code-interpreter / browser | Firecracker microVM | Managed |

::: {.fragment}
Fastest start (Daytona, containers) vs. strongest isolation (Firecracker microVMs) — the core trade-off.
:::

## The common thread {background-color="#eef1f4" .smaller}

::: {.incremental}
- Most are **closed SaaS** or **single-cloud**
- Underneath, it's the **same few isolation techs** — Firecracker · gVisor · Kata
- Run your own — multi-tenant, on your cluster — and you must choose: **runtime + control plane + a k8s-native API**
:::

::: {.fragment}
That's the rest of this talk → **① which secure runtime** (measured) · **② OpenSandbox** · **③ Agent Sandbox**
:::
...
![](images/runtimes/isolation-spectrum.png){width="82%" fig-align="center" style="border-radius:8px;border:1px solid #ddd;"}
...
## 1.3 · Kata Containers — the microVM framework {background-color="#f5f2ec" .smaller}

![](images/runtimes/kata.svg){.absolute top="20" right="30" height="60"}

**CNCF project** — "the speed of containers, the security of VMs".

::: {.incremental}
- Makes a **microVM look like a container** to Kubernetes (OCI / CRI, containerd & CRI-O shims)
- Each pod runs in a lightweight VM with its **own guest kernel** — hardware-virt boundary
- **Pluggable VMM (hypervisor)** → this is where FC / CLH / QEMU plug in
- Selected per pod via a **`RuntimeClass`**: `kata-fc`, `kata-clh`, `kata-qemu`
:::

::: {.fragment}
So the next three contenders are all **Kata + a different VMM**.
:::

## 1.4 · Firecracker — `kata-fc` {background-color="#f5f2ec" .smaller}

![](images/runtimes/firecracker.svg){.absolute top="20" right="30" height="64"}

**AWS's minimal microVM monitor (Rust)** — the engine behind Lambda & Fargate.

::: {.incremental}
- Boots a **real guest Linux kernel** inside a **KVM microVM**
- **Minimal device model** → tiny attack surface, very fast VMM boot (~125–200 ms)
- Boundary: **hardware virtualization**
- Needs: `/dev/kvm` + nested-virt + containerd **devmapper** snapshotter
:::

::: {.fragment}
**Our take:** fastest hot-path + best steady-state, strongest isolation — but the heaviest to operate.
:::

## 1.5 · Cloud Hypervisor — `kata-clh` {background-color="#f5f2ec" .smaller}

![](images/runtimes/cloud-hypervisor.png){.absolute top="20" right="30" height="64" style="border-radius:8px;"}

**A modern, Rust-based VMM** (Intel + community), cloud-native focus.

::: {.incremental}
- Real guest kernel in a **KVM microVM**, richer device model than FC
- Boundary: **hardware virtualization**
- Needs: `/dev/kvm` + nested-virt (**no** devmapper requirement)
- Cleaner path for **GPU / PCI passthrough**
:::

::: {.fragment}
**Our take:** a solid Firecracker alternative / fallback — but mid-pack on latency in our tests.
:::

## 1.6 · QEMU — `kata-qemu` {background-color="#f5f2ec" .smaller}

![](images/runtimes/qemu.svg){.absolute top="20" right="30" height="64"}

**The original, full-featured VMM & emulator** — the compatibility baseline.

::: {.incremental}
- Real guest kernel in a **KVM microVM** with a **full device model**
- Boundary: **hardware virtualization**
- Needs: `/dev/kvm` + nested-virt
- **Broadest** device / hardware support of the four
:::

::: {.fragment}
**Our take:** maximum compatibility, but the **most overhead** — slowest on every axis we measured.
:::

## 1.7 · gVisor — `runsc` {background-color="#f5f2ec" .smaller}

![](images/runtimes/gvisor.png){.absolute top="20" right="30" height="64"}

**Google's userspace kernel (Go)** — used by GKE, App Engine, Cloud Run.

::: {.incremental}
- **No VM at all** — intercepts syscalls in userspace and re-implements the kernel
- Boundary: **syscall ABI** (~80% coverage)
- Needs: **nothing special** — runs on any Linux node
- **Zero** per-pod overhead; some syscall-heavy workloads unsupported
:::

::: {.fragment}
**Our take:** best cold-start, density, and ops simplicity — pays a syscall tax on hot, compute-heavy work.
:::

## 1.8 · How we measured {background-color="#f5f2ec" .smaller}
...
## 1.9 · ★ Headline results {background-color="#f5f2ec" .smaller .scrollable}
...
## 1.10 · Where do the seconds go? {background-color="#f5f2ec" .smaller}
...
## 1.11 · Runtime performance under load {background-color="#f5f2ec" .smaller .scrollable}
...
## 1.12 · The ops reality {background-color="#f5f2ec" .smaller}
...
## 1.13 · Capacity & cost {background-color="#f5f2ec" .smaller}
...
## 1.14 · Verdict {background-color="#f5f2ec" .smaller}

Not "X wins" — **route per workload**:

::: {.incremental}
- 🔥 **Hot interactive path** → **kata-fc** — fastest dispatch + steady-state, strongest isolation
- ⚡ **Cold / per-request / high-density** → **gVisor** — −30% cold latency, zero overhead, commodity nodes
- 🚫 **kata-qemu** → off the table — no axis where it wins
:::
...
::: {.notes}
The takeaway isn't 'X wins' — it's a validated, measured routing table. Firecracker is our default for
the hot interactive path (fastest where users feel it, strongest isolation). gVisor is the right backend
for cold, per-request, or high-density workloads. QEMU is off the table. And crucially — the platform can
route per-workload from the same control plane. That flexibility is a product feature, and the bridge
into Section 2.
:::

## 1.15 · Full routing table {background-color="#f5f2ec" .smaller .scrollable}

| Workload | Use | Why |
|---|---|---|
| Interactive agent, rapid tool loops | **kata-fc** | Best dispatch + steady-state, real kernel |
| Fresh sandbox per request (no pool) | **gVisor** | −30% cold latency, no nested-virt |
| High density / cost-sensitive fleet | **gVisor** | Zero overhead + commodity nodes |
| Needs ptrace / io_uring / mount | **kata-fc / clh** | gVisor's ~80% syscall coverage fails these |
| Highest-trust isolation | **kata-fc** | Hardware-virt boundary |
| GPU / PCI passthrough / fallback | **kata-clh** | Works if FC support drops |
| — anything — | kata-qemu | No axis where it wins |

## 1.16 · Compatibility matrix {background-color="#f5f2ec" .smaller .scrollable}
...
| iptables `nat` (egress sidecar) | Yes | **No** | Yes | Yes | Yes |

::: {.fragment}
Source: [OpenSandbox — Secure Container Runtime guide](https://github.com/opensandbox-group/OpenSandbox/blob/main/docs/guides/secure-container.md#compatibility-matrix)
:::
...
:::: {.columns}
::: {.column width="50%"}
![Coding agents — Claude Code](images/opensandbox/scenario-claude-code.jpg){width="94%" style="border-radius:8px;border:1px solid #ddd;"}
:::
::: {.column width="50%"}
![Browser automation — Playwright](images/opensandbox/scenario-playwright.png){width="94%" style="border-radius:8px;border:1px solid #ddd;"}
:::
::::

::: {.notes}
Coding agents and browser automation — two of the most common sandbox workloads.
:::

## Typical scenarios (cont.) {background-color="#f4f2fb"}

:::: {.columns}
::: {.column width="50%"}
![Remote development — VS Code Web](images/opensandbox/scenario-vscode.jpg){width="94%" style="border-radius:8px;border:1px solid #ddd;"}
:::
::: {.column width="50%"}
![RL training & evaluation](images/opensandbox/scenario-rl.jpg){width="94%" style="border-radius:8px;border:1px solid #ddd;"}
:::
::::

::: {.notes}
Remote dev and RL training. Also: AI code execution and enterprise isolation
(secure runtimes + egress + ingress). Every one is just a sandbox with a
different image and entrypoint.
:::
...
::: {.fragment}
**Why not native k8s NetworkPolicy?**

- **Labels unpredictable** — Pod labels are platform-injected, not user-controllable → can't select per-tenant
- **Static vs. churn** — sandboxes are created/destroyed constantly; NetworkPolicy is a static declaration
- **Wrong granularity** — Pod-set selectors, but each sandbox needs its own "deny all others" domain
- **No FQDN / L7** — L3/L4 & IP-based; can't allowlist domains — the control point must live *in* the sandbox

[Network Isolation — why NetworkPolicy is insufficient](https://github.com/opensandbox-group/OpenSandbox/blob/main/docs/architecture/network-isolation.md)
:::
...
## Flow: creating a sandbox {background-color="#ffffff"}

![](images/opensandbox/creation-flow.svg){.r-stretch fig-align="center"}
...
## Architecture {background-color="#ffffff"}

![](images/agent-sandbox/architecture.svg){.r-stretch fig-align="center"}
...
# Any questions? {background-color="#f4f2fb"}
